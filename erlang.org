        ...
        (string-append
         ...
         (if (array-type? type)
             (string-append
              ...
              (indent-line (+ base-indent 8) accessor " /= nil, " accessor " /= undefined ->")
              (indent-line (+ base-indent 12) "{Tmp, Cnt} = tightrope:padding(LastTag, Tag, Buf),")
              (indent-line (+ base-indent 12) "set_fields(X, Tag, Tag + 1, <<Tmp/binary, 0:16>>, [Tag | DTags], Count + Cnt + 1);")
              (indent-line (+ base-indent 8) "true ->")
              (indent-line (+ base-indent 12) "set_fields(X, LastTag, Tag + 1, Buf, DTags, Count)")
             ...
             (if (or (eq? type 'string) (custom-type? type))
                 ...
                 (string-append
                  (indent-line (+ base-indent 4) "if")
                  (indent-line (+ base-indent 8) accessor " /= 0, " accessor " /= undefined ->")
                  (indent-line (+ base-indent 12) "{Tmp, Cnt} = tightrope:padding(LastTag, Tag, Buf),")
                  (indent-line (+ base-indent 12) "if")
                  (if (eq? type 'byte)
                      (indent-line (+ base-indent 16) "0 < " accessor " ->")
                      (indent-line (+ base-indent 16) "0 < " accessor ", " accessor " < 16383 ->"))
                  (indent-line (+ base-indent 20) "set_fields(X, Tag, Tag + 1, <<Tmp/binary, ((" accessor " + 1) * 2):16>>, DTags, Count + Cnt + 1);")
                  (indent-line (+ base-indent 16) " true ->")
                  (indent-line (+ base-indent 20) "set_fields(X, Tag, Tag + 1, <<Tmp/binary, 0:16>>, [Tag | DTags], Count + Cnt + 1)")
  ...
  (define (generate-erlang-set-fields-recursive struct)
    (let ((name (>erlang-name (struct-name struct)))
      ...
      (let* ((spec (string-append "-spec set_fields(#" name "{}, non_neg_integer(), non_neg_integer(), binary(), [non_neg_integer()], non_neg_integer()) -> {binary(), [non_neg_integer()]}.\n"))
             (sign "set_fields(X, LastTag, Tag, Buf, DTags, Count) ->\n")
             (max-tag (number->string (apply max (map (lambda(x) (field-tag x)) fields))))
             (body (string-append
                    (indent-line 04 "if")
                    (indent-line 08 "Tag > " max-tag " ->")
                    ...
                    (indent-line 16 "_ ->")
                    (indent-line 20 "set_fields(X, LastTag, Tag + 1, Buf, DTags, Count)")
                    (indent-line 12 "end")
                    (indent-line 4 "end."))))
