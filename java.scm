        ...
        (let* ((field (car fields))
          ...
          (loop
           ...
           (cons (generate-java-encoder-init
                  ...
                  (string-field-count fields)
                  (number->string objidx)
                  (custom-field-count fields)
                  (number->string strarridx)
                  (string-array-count fields)
                  (number->string objarridx)
                  (custom-array-count fields))
                 inits))))))
  ...
  (let loop ((fields (struct-fields struct))
    ...
    (if (null? fields)
        (let ((loop-start "for (short i = 0; i < tlen; i ++) {")
              (loop-stop "}")
              (insert-skip-field "if (i > 0) {if (tags[i - 1] + 1 != tags[i]) {buf.putShort((short)((tags[i] - tags[i - 1] - 1 - 1) * 2 + 1));}}")
